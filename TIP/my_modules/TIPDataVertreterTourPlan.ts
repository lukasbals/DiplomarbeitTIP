      ...
      request.get(
        ...
        (error, response, body: string): void => {
          ...
          TIPDatabase.getDB().serialize((): void => {
            var insertStmt = TIPDatabase.getDB().prepare("insert into touren_plan (client_id, tour_name,von, bis) values (?, ?, ?, ?)");
            var updateStmt = TIPDatabase.getDB().prepare("update touren_plan set tour_name=?, von=?, bis=? where client_id=?");

            var insertCount = 0;
            var updateCount = 0;

            data.forEach((val: any): void => {
              TIPDatabase.getDB().get("select count(*) as result from touren_plan where client_id = ?", [val.client_id], (error, row): void => {
                if (row.result > 0) {
                  updateCount++;
                  updateStmt.run([val.Id, val.TourName, val.Von, val.Bis]);
                } else {
                  insertCount++;
                  insertStmt.run([val.ClientId, val.TourPlan, val.Von, val.Bis]);
                }
              });
            });
